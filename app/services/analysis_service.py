...
class SmartAnalysisService:
    ...
    def _assess_relevance(self, query: str, context_chunks: List[Dict]) -> float:
        ...
        ChromaDB cosine distances are used: lower distance = more similar.
        Returns a score in [0, 1] (0.5 neutral).
        """
        ...
        for chunk in context_chunks:
            ...
            if score is not None:
                ...
                relevance_scores.append(max(0.0, min(1.0, 1.0 - float(distance) / 2.0)))
            ...
            try:
                ...
                if fallback_result:
                    ...
                    if rag_available and not force_fallback:
                        fallback_reason = fallback_reason or "RAG unavailable or unsuccessful, using fallback"
    ...
    async def _analyze_with_rag(self, query: str) -> Optional[Dict]:
        ...
        Returns a dict with:
            success: bool
            response / chunks / confidence / error
        """
        if self.rag_service is None:
        ...
        try:
            ...
            metadatas = (search_result.get("metadatas") or [[]])[0] or []
            distances = (search_result.get("distances") or [[]])[0] or []

            if not documents:
                return {"success": False, "error": "No relevant documents found"}

            # Build clean labeled context (only top-k final chunks go to the LLM)
            final_k = getattr(self.rag_service, "final_context_k", None) or 3
            context = self.rag_service.build_context(search_result, limit=final_k)

            # Structured chunk info for the audit trail (top final_k chunks)
            rag_chunks = []
            for doc, meta, dist in zip(documents[:final_k], metadatas[:final_k], distances[:final_k]):
                rag_chunks.append({
            ...
            # Generate response using the provider (streaming adapter handles sync/async)
            response = self.rag_service.generate_stream_response(
            ...
            # Confidence: combination of retrieval distance and response presence
            avg_dist = sum(distances[:final_k]) / len(distances[:final_k]) if distances else 1.0
            dist_confidence = max(0.3, min(0.95, 1.0 - avg_dist / 2.0))
            confidence = min(0.95, (dist_confidence + 0.6) / 2)
    ...
    async def _analyze_without_rag(self, query: str) -> Optional[Dict]:
        ...
        try:
            ...
            # Extract actual text content from the provider completion
            content = self.rag_service._extract_content(response)
            confidence = min(0.8, 0.5 + len(str(content)) / 1000 * 0.1)

            return {
                "success": True,
                "response": str(content) if content else "I don't know",
                "confidence": confidence
            }
